 ...
 * rutas sin sesión.
 *
 * ## Por qué está duplicado en `vendi-admin` y aquí, y no en `ui-kit`
 *
 * Porque no puede estar en `ui-kit`: el componente inyecta `Notificador`, que
 * vive en `data-access`, y la frontera de ADR-011 prohíbe expresamente ese
 * import desde `ui-kit` (lo hace cumplir su `eslint.config.js`). La alternativa
 * —un componente presentacional que reciba el aviso por input— movería a cada
 * app exactamente el mismo `effect`, así que no ahorra nada. Son ~50 líneas y
 * cada app puede decidir distinto cómo enseña sus avisos; el móvil, por
 * ejemplo, querrá algo que se vea con el dedo encima de la pantalla.
 */
@Component({
  selector: 'vd-avisos',
  template: '',
