...
export class ElegirNegocioComponent {
  ...
  elegir(alias: string): void {
    if (!this.auth.selectTenant(alias)) {
      ...
      // todos modos llevaría al POS a pedir datos con el tenant anterior, que
      // es peor que quedarse en el selector.
      return;
    }
    // El `catch` no es ceremonia: si alguien reorganiza las rutas y `/` deja de
    // existir, `navigate()` devuelve una promesa rechazada que nadie observa y
    // el error aparece como "unhandled rejection" sin decir dónde.
    this.router.navigate(['/']).catch((error: unknown) => {
      console.error('No se pudo abrir el POS tras elegir el negocio.', error);
    });
  }
